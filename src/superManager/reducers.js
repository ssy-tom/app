...
function setClassOrRewards(state = true, action) {
    switch (action.type) {
        case "setClassOrRewards":
...
function pageRewrds(state = {}, action) {
    switch (action.type) {
        case "page_rewrds":
            return action.text || {};
            break;
...
function getStudentRecord(state = {status: 1, data: null}, action) {
    switch (action.type) {
        ...
        case "get_stu":
            ...
            action.text.data.forEach(function (obj, index) {
                //console.log("rrr", obj.token, today);
                if (obj.token == today) {
                    todayData.push(obj);
                } else {
...
const superManager = combineReducers({
    clickLinkage,
    pageRewrds,
    getStudentRecord,
    setClassOrRewards,
